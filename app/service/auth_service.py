...
class KeycloakAuthService:
    ...
    async def validate_token_for_middleware(self, token: str) -> bool:
        ...
        logger.debug("Starting middleware token validation")

        try:
            result = self.keycloak_openid.introspect(token)

            if result and result.get('active', False):
                logger.debug("Token is active in middleware")
                return True
            else:
                ...
                if is_active:
                    logger.debug("Exchanged token is active in middleware")
                    return True
                else:
